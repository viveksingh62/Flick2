  ...
  return (
    <>
        ...
        <div className="hidden md:flex flex-1 max-w-md mx-4">
          <form onSubmit={handleSearch} className="relative w-full">
            <input
              ...
              placeholder="Search..."
              className="px-3 py-2 pr-10 rounded-lg border border-gray-300 w-full text-white bg-[#1A1A40]"
            />
            <button type="submit">
      ...
      <motion.div
        ...
        className="fixed top-0 right-0 h-full w-64 bg-[#0A2540] shadow-lg z-50"
      >
        ...
        <div className="flex flex-col p-4 space-y-4">
          ...
          {user && (
            <Link
              ...
              className="px-4 py-2 text-center bg-[#1A3A5A] text-white rounded-lg hover:bg-emerald-500 transition"
            >
              <FontAwesomeIcon icon={faUser} /> My Profile
            </Link>
          )}
          ...
          {/* Mobile Links */}
          <button onClick={() => handleNavClick("/prompt")} className="text-white hover:text-emerald-400">Create</button>
          <button onClick={() => handleNavClick("/about")} className="text-white hover:text-emerald-400">About</button>
          ...
          {!user ? (
            <>
              <button onClick={() => handleNavClick("/login")} className="px-4 py-2 text-center bg-[#1A3A5A] text-white rounded-lg hover:bg-emerald-500 transition">Login</button>
              <button onClick={() => handleNavClick("/signup")} className="px-4 py-2 text-center bg-[#1A3A5A] text-white rounded-lg hover:bg-emerald-500 transition">Signup</button>
            </>
          ) : (
            <button onClick={() => { logout(); setIsOpen(false); }} className="px-4 py-2 text-center bg-[#1A3A5A] text-white rounded-lg hover:bg-emerald-500 transition">Logout</button>
          )}
        </div>
